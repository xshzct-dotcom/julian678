...
function renderRiver(){
  ...
  stream.innerHTML = indices.map(function(pi, i){
    ...
    return '<div class="polaroid" data-idx="' + pi + '" style="transform:rotate(' + rot + 'deg)" data-label="' + esc(p._albumTitle||'') + '">' +
      '<img src="' + thumb(p) + '" alt="" decoding="async" onload="var p=this.parentElement;if(p)p.classList.remove(\'polaroid-loading\')" onerror="var p=this.parentElement;if(p)p.classList.add(\'polaroid-broken\')">' +
    '</div>';
  }).join('');
